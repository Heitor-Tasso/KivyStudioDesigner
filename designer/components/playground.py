...
from utils.toolbox_widgets import toolbox_widgets as widgets_common

from utils.utils import (
    FakeSettingList, get_app_widget,
...
<PlaygroundDragElement>:
    ...
    canvas:
        ...
        Line:
            points:
                [self.x, self.y, (self.center_x-dp(20)), self.y,\
                self.center_x, (self.y-dp(20)), (self.center_x+dp(20)), self.y,\
                self.right, self.y, self.right, self.top, self.x, self.top]
            close: True
            width: dp(1.5)
        Color:
            rgb: bgcolor
...
class PlaygroundDragElement(BoxLayout):
    ...
    def on_touch_move(self, touch):
        ...
        if self.is_intersecting_playground(touch.x, touch.y):
            target = self.playground.try_place_widget(self.widget, touch.x, touch.y)

        # if is targeting widget tree
        elif self.is_intersecting_widgettree(touch.x, touch.y):
            pos_in_tree = self.widgettree.tree.to_widget(touch.x, touch.y)
            ...
            if node:
                ...
                if node.node == self.widget:
                    ...
                    while node and node.node != self.playground.sandbox:
                        widget = node.node
                        if self.playground.allowed_target_for(widget, self.children):
        ...
        if self.is_intersecting_playground(touch.x, touch.y):
            ...
            if node:
                ...
                while widget and widget != self.playground.sandbox:
                    if self.playground.allowed_target_for(widget, self.widget):
                        target = widget
            ...
            if self.can_place or self.playground.root is None:
                ...
                if self.drag_type == 'dragndrop':
                    ...
                    # adding by widget tree
                        ...
                        self.playground.add_widget_to_parent(self.widget, target)
...
class Playground(ScatterPlane):
    ...
    def load_widget(self, widget_name, update_kv_lang=True):
        ...
        if self.root is None or self.root_app_widget is None or widget_name not in widgets:
        ...
        if target.kv_path != self.root_app_widget.kv_path and not self.kv_code_input.saved:
    ...
    def _perform_load_widget(self, widget_name, update_kv_lang=True):
        ...
        try:
            ...
            if update_kv_lang:
                ...
                if kv_path:
                    with open(kv_path, encoding='utf-8') as file:
                        self.kv_code_input.text = file.read()
                        file.close()
                else:
                    msg = f'Could not found the associated .kv file with {widget_name} widget'
            ...
            show_message(f'Failed to load {widget_name} widget', 5, 'error')
    ...
    def on_reload_kv(self, kv_lang_area, text, force):
        ...
        try:
            ...
            if self.root_name and self.root_name not in proj.app_widgets:
                ...
                msg = f'The {self.root_name} is not available. Displaying another widget'
                show_message(msg, 5, 'info')
            ...
            show_message(f'Failed to load {self.root_name} widget', 5, 'error')
    ...
    def load_widget_from_file(self, kv_path):
        ...
        with open(kv_path, 'r', encoding='utf-8') as file:
            self.kv_code_input.text = file.read()
            file.close()
        ...
        if isinstance(target, FloatLayout) or \
            ...
            if not from_undo:
                self.undo_manager.push_operation(
                    WidgetDragOperation(
                        widget, target, self.drag_operation[1],
                        self.drag_operation[2], self, extra_args=extra_args))
            ...
            if 'prev_index' in extra_args:
                self.kv_code_input.shift_widget(widget, extra_args['prev_index'])
            else:
    ...
    def add_widget_to_parent(self, widget, target, from_undo=False, from_kv=False, kv_str='', extra_args={}):
        ...
        if not from_kv:
            ...
            self.kv_code_input.add_widget_to_parent(widget, target, kv_str=kv_str)
        
        if not from_undo:
            root = App.get_running_app().root
            root.undo_manager.push_operation(WidgetOperation('add', widget, target, self, ''))
    ...
    def generate_kv_from_args(self, widget_name, kv_dict, *args):
        ...
        for key in kv_dict.keys():
            ...
            if isinstance(value, basestring):
                value = f"'{value}'"
            kv += f'{indent}{key}: {value}'

        return kv

    def get_playground_drag_element(self, instance, widget_name, touch, default_args, extra_args, *args):
    ...
    def cleanup(self):
        ...
        if self.root:
            self.remove_widget_from_parent(self.root, from_undo=True, from_kv=True)
    ...
    def find_target(self, x, y, target, widget=None):
        ...
        for child in target.children:
            ...
            for _widget in widgets_common:
                if _widget[0] == type(child).__name__ and _widget[1] == 'complex':
    ...
    def do_copy(self, for_drag=False):
        ...
        if base_widget:
            ...
            for prop in props:
                ...
                setattr(self.widget_to_paste, prop, getattr(base_widget, prop))
        ...
        if parent and self.widget_to_paste:
            ...
            while parent:
                if isinstance(parent, Layout):
                    if not is_child_custom or root_widget == parent:
                        break
            ...
            if parent is not None:
                self.add_widget_to_parent(
                    self.widget_to_paste, parent,
    ...
    def do_cut(self):
        ...
        if base_widget and base_widget.parent:
            ...
            get_txt_widget = self.kv_code_input.get_widget_text_from_kv
            self._widget_str_to_paste = get_txt_widget(base_widget, None)
