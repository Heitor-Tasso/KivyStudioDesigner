__all__ = [
    'ListView', 'ListItemButton', 'ListAdapter',
    'SuggestionItem', 'CompletionListView',
    'CompletionBubble']
...
class ListView(Button):
    adapter = ObjectProperty(None)

class ListItemButton(Button):
    pass

class ListAdapter(Widget):
...
class CompletionBubble(Bubble):
    ...
    def show_completions(self, completions, force_scroll=False):
        ...
        '''
        if completions == []:
            fake_completion = type('obj', (object,), {'name': 'No suggestions', 'complete': ''})
    ...
    def reposition(self, pos, line_height):
        ...
        if self.y == pos[1] - self.height - line_height:
            self.arrow_pos = f'top_{_get_hpos()}'
        else:
            self.arrow_pos = f'bottom_{_get_hpos()}'
...
if __name__ == '__main__':
    ...
    class Test(FloatLayout):
        ...
        def show_bubble(self):
            source = 'datetime.da'
            #             Old jedi.Script args
            # --> print(source, 3, len('datetime.da')) <--
