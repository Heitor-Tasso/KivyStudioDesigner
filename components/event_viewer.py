__all__ = [
    'EventHandlerTextInput', 'NewEventTextInput',
    'EventLabel', 'EventViewer']

from utils.utils import get_current_project, get_designer, show_message
from components.property_viewer import PropertyLabel, PropertyViewer
from uix.info_bubble import InfoBubble
...
class EventHandlerTextInput(TextInput):
    ...
    def show_drop_down_for_widget(self, widget):
        ...
        for func in list_funcs:
            if '__' not in func and hasattr(getattr(widget, func), '__call__'):
                btn = Button(
                    text=func, size_hint=(None, None),
                    size=(dp(100), dp(30)), shorten=True)

                self.dropdown.add_widget(btn)
                btn.bind(on_release=lambda btn: self.dropdown.select(btn.text))
                btn.text_size = [(btn.size[0]-dp(4)), btn.size[1]]
                btn.valign = 'middle'
    ...
    def on_text(self, instance, value):
        ...
        set_event_handler = self.kv_code_input.set_event_handler
        set_event_handler(self.eventwidget, self.eventname, self.text)
...
class EventViewer(PropertyViewer):
    ...
    def discover(self, value):
        ...
        for event in events:
            ...
            self.prop_list.add_widget(EventLabel(text=event))
            self.prop_list.add_widget(ip)
        ...
        if is_custom_widget:
            ...
            msg = 'Type and press enter to \ncreate a new event'
            self.prop_list.add_widget(EventLabel(text=msg))

            msg = 'Type and press enter to create a new event'
            txt = NewEventTextInput(multiline=False, info_message=msg)
            
            txt.bind(on_create_event=self.create_event)
            self.prop_list.add_widget(txt)
    ...
    def create_event(self, txt):
        ...
        self.designer_tabbed_panel.open_file(py_file, rel_path, switch_to=True)
    ...
    def _add_event(self, *args):
        ...
        pos = -1
        r_iter = rf'class\s+{type(self.widget).__name__}\(.+\):'
        for searchiter in re.finditer(r_iter, py_code_input.text):
            pos = searchiter.end()
        ...
        if found_events:
            ...
            py_code_input.insert_text(f', "{txt.text}" ')

            py_code_input.cursor = (0, (events_row+1))
            
            line = f'\n    def {txt.text}(self, *args):\n        pass\n'
            py_code_input.insert_text(line)
        else:
            line = dedent(f'''
